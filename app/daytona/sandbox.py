...
def get_daytona_settings():
    ...
    if daytona_settings is None:
        raise RuntimeError(
            "Daytona sandbox is not configured. Add a [daytona] section to "
            "config/config.toml before using sandbox tools."
        )
    return daytona_settings
...
def get_daytona_client() -> Daytona:
    ...
    daytona_settings = get_daytona_settings()
    ...
    if daytona_config.api_key:
        ...
        logger.warning("No Daytona API key found in configuration")

    if daytona_config.server_url:
        logger.info(f"Daytona server URL set to: {daytona_config.server_url}")
    else:
        logger.warning("No Daytona server URL found in configuration")

    if daytona_config.target:
        logger.info(f"Daytona target set to: {daytona_config.target}")
    else:
        logger.warning("No Daytona target found in configuration")
...
def create_sandbox(password: str = None, project_id: str = None):
    ...
    daytona_settings = get_daytona_settings()
    if password is None:
        password = daytona_settings.VNC_password

    logger.info("Creating new Daytona sandbox environment")
    logger.info("Configuring sandbox with browser automation environment variables")
