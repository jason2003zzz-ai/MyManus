SYSTEM_PROMPT = (
    ...
    " Treat search results and snippets only as discovery. For factual web research, open or fetch the strongest first-party source and inspect the relevant original page, navigation, or site search before answering."
    " Never infer that information does not exist merely because a search result page omits it. Before any negative conclusion such as 'not found' or 'not publicly available', inspect at least one direct source page and perform another relevant retrieval step with an alternate query or page path."
...
For web research, search first for discovery, then open or fetch the strongest primary source. Search snippets alone cannot prove either a positive fact or the absence of information. Inspect same-site navigation and likely intent pages when the exact content is not on the landing page.
